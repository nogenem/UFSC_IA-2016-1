...
public class RegulatorServer implements Runnable {
	 ...
	 *
	 * @param motorCtrl
	 *            a MotorController that will simulate robot movement
	 * @param sensor
	 *            sensor that will provide line position
	 */
	...
	/**
	 * Thread that periodicaly updates motors' speed acording to the line
	 * position contains the regulator loop itself
	 */
	public void run() {
		...
		while (Thread.currentThread() == timer) {
			...
			// 		sumLinePositions = 0;
			// } else {
			// 		out.println(position + " " + sumLinePositions + " " +
	...
	public void listenSocket() {
		try {
			...
			System.out.println("Server is listening on " + server.getInetAddress().getHostName() + ":" + 4321);
		} catch (IOException e) {
